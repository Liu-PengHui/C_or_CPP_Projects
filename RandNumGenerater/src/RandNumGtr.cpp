...
INT_PTR CALLBACK    UpdateLogDialog(HWND, UINT, WPARAM, LPARAM);
...
long long GenerateRandomNumber(long long min, long long max)
{
    ...
    if (min >= 0) {
        range = static_cast<unsigned long long>(max) - static_cast<unsigned long long>(min);
    }
    else if (max <= 0) {
        range = static_cast<unsigned long long>(max - min);
    }
    else {
        range = static_cast<unsigned long long>(max) + static_cast<unsigned long long>(-min);
    }
    ...
    else if (range <= 0xFFFF) {  // <= 65535
        bytesNeeded = 2;
        mask = 0xFFFF;
    }
    else if (range <= 0xFFFFFFFF) { // <= 42.9亿
        bytesNeeded = 4;
        mask = 0xFFFFFFFF;
    }
    ...
    do {
        BCryptGenRandom(hAlg, (PUCHAR)&randomValue, bytesNeeded, 0);
        randomValue &= mask;  // 根据实际字节数屏蔽
    ...
    case WM_COMMAND:
    {
        ...
        else
        {
            ...
            case IDM_ABOUT:
                ...
                break;
            case IDM_UPDATELOG:
                DialogBox(hInst, MAKEINTRESOURCE(IDD_UPDATELOG), hWnd, UpdateLogDialog);
                break;
            case IDM_SETTINGS:
...
// 更新日志对话框回调函数
INT_PTR CALLBACK UpdateLogDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
    switch (message)
    {
    case WM_INITDIALOG:
    {
        const WCHAR* logContent =
            L"========================================\r\n"
            L"     RandNumGenerator 更新日志\r\n"
            L"========================================\r\n\r\n"
            L"【2026/5/7】版本 3.2.1\r\n"
            L"  • 新增：更新日志选项\r\n"
            L"  • 优化：随机数生成性能提升\r\n"
            ...
            L"  • 优化：界面视觉效果改进\r\n\r\n"
            L"========================================\r\n"
            L"      感谢使用 RandNumGenerator！\r\n"
            L"========================================";

        HWND hEditLog = GetDlgItem(hDlg, IDC_EDIT_LOG);
        if (hEditLog)
        {
            SetWindowTextW(hEditLog, logContent);
            SendMessage(hEditLog, EM_SETREADONLY, TRUE, 0);
        }
        return (INT_PTR)TRUE;
    }
    ...
    case WM_SHOWWINDOW:
    {
        ...
        if (wParam == TRUE)  // 正在显示
        {
            ...
            {
                // 将光标设置到开头并清除选择
                SendMessage(hEditLog, EM_SETSEL, 0, 0);
            }
        }
        return (INT_PTR)FALSE;
    }
